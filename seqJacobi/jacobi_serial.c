 ...
 *************************************************************/
inline __attribute__((always_inline)) double one_jacobi_iteration(double xStart, double yStart,
...
#define DST(XX,YY) dst[(YY)*maxXCount+(XX)]
    int x, y;
    double fX, fY, fXSquare, fYSquare;
    double error = 0.0;
    double updateVal;
    double f;
    ...
    for (y = 1; y < (maxYCount-1); y++)
    {
        ...
        for (x = 1; x < (maxXCount-1); x++)
        {
            ...
            DST(x,y) = curVal - omega*updateVal;
